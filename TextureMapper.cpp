...
int TextureMapper::Tixi(std::vector<std::vector<int>> completenessPatches, std::vector<std::vector<int>> coherencePatches, int c /*color channel*/) {
    ...
    int sum3 = 0;
    for (int k = 0; k < N; k++) {
        //Mk(Xi->k) RGB color of the kth target image at pixel Xi->j, i.e., the result of projecting texture k to camera i
        // (Xi->j is pixel position projected from image i to j)
        sum3 += Mk(Xi->k);
    }
